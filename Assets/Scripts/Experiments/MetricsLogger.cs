...
public struct Experiment001EpisodeMetrics
{
    public int mazeSeed;
...
public sealed class MetricsLogger
{
    ...
    public MetricsLogger(string relativePath, bool enabled = true)
    {
        Enabled = enabled;
    ...
    public int LogEpisode(in Experiment001EpisodeMetrics metrics)
    {
        if (!Enabled)
            return 0;
        ...
        Debug.Log($"[MetricsLogger] episode_id={episodeId} -> {_filePath}");
        return episodeId;
    }
    ...
    static string FormatRow(int episodeId, in Experiment001EpisodeMetrics metrics)
    {
        return string.Join(",",
            ...
            metrics.collisions.ToString(CultureInfo.InvariantCulture),
            metrics.pathLength.ToString("F2", CultureInfo.InvariantCulture),
            metrics.coveragePercent.ToString("F2", CultureInfo.InvariantCulture),
