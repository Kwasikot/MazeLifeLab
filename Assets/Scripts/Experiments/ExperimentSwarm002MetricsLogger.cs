...
public struct ExperimentSwarm002EpisodeMetrics
{
    public int seed;
    ...
    public int obstacleContacts;
    public int nearObstacleSteps;
    public float meanObstacleDistance;
    public float fragmentationIndex;
    public EpisodeTerminationReason terminationReason;
}

public sealed class ExperimentSwarm002MetricsLogger
{
    const string CsvHeader =
        "episode_id,seed,agent_count,obstacle_count,steps,mean_speed,mean_neighbor_distance,cohesion_index," +
        "separation_violations,boundary_hits,coverage_volume_percent,obstacle_contacts,near_obstacle_steps," +
        "mean_obstacle_distance,fragmentation_index,termination_reason";

    readonly string _filePath;
    readonly object _writeLock = new object();
    int _nextEpisodeId;

    public ExperimentSwarm002MetricsLogger(string relativePath, bool enabled = true)
    {
        Enabled = enabled;
    ...
    public int LogEpisode(in ExperimentSwarm002EpisodeMetrics metrics)
    {
        if (!Enabled)
            return 0;
        ...
        Debug.Log($"[EXP-SWARM-002 Metrics] episode_id={episodeId} -> {_filePath}");
        return episodeId;
    }
    ...
    static string FormatRow(int episodeId, in ExperimentSwarm002EpisodeMetrics metrics)
    {
        return string.Join(",",
            ...
            metrics.obstacleCount.ToString(CultureInfo.InvariantCulture),
            ...
            metrics.obstacleContacts.ToString(CultureInfo.InvariantCulture),
            metrics.nearObstacleSteps.ToString(CultureInfo.InvariantCulture),
            metrics.meanObstacleDistance.ToString("F3", CultureInfo.InvariantCulture),
            metrics.fragmentationIndex.ToString("F3", CultureInfo.InvariantCulture),
