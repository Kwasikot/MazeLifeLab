...
public struct Experiment004EpisodeMetrics
{
    public int mazeSeed;
...
public sealed class MultiAgentMetricsLogger
{
    const string CsvHeader =
        "episode_id,maze_seed,algorithm,agent_count,success,steps,steps_to_first_goal,agents_at_goal," +
        "total_collisions,total_path_length,team_coverage_percent,overlap_percent,termination_reason";

    readonly string _filePath;
    readonly object _writeLock = new object();
    int _nextEpisodeId;

    public MultiAgentMetricsLogger(string relativePath, bool enabled = true)
    {
        Enabled = enabled;
    ...
    public int LogEpisode(in Experiment004EpisodeMetrics metrics)
    {
        if (!Enabled)
            return 0;
        ...
        Debug.Log($"[MultiAgentMetricsLogger] episode_id={episodeId} -> {_filePath}");
        return episodeId;
    }
    ...
    static string FormatRow(int episodeId, in Experiment004EpisodeMetrics metrics)
    {
        return string.Join(",",
