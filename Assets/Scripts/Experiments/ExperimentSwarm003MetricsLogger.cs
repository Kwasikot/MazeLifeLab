...
public struct ExperimentSwarm003EpisodeMetrics
{
    public int seed;
    public string explorationMode;
    ...
    public int newVoxelsDiscovered;
    public float revisitRatio;
    public float meanVisitCount;
    public float explorationEfficiency;
    public int frontierBiasSteps;
    public EpisodeTerminationReason terminationReason;
}

public sealed class ExperimentSwarm003MetricsLogger
{
    const string CsvHeader =
        "episode_id,seed,exploration_mode,agent_count,steps,mean_speed,mean_neighbor_distance,cohesion_index," +
        "separation_violations,boundary_hits,coverage_volume_percent,new_voxels_discovered,revisit_ratio," +
        "mean_visit_count,exploration_efficiency,frontier_bias_steps,termination_reason";

    readonly string _filePath;
    readonly object _writeLock = new object();
    int _nextEpisodeId;

    public ExperimentSwarm003MetricsLogger(string relativePath, bool enabled = true)
    {
        Enabled = enabled;
    ...
    public int LogEpisode(in ExperimentSwarm003EpisodeMetrics metrics)
    {
        if (!Enabled)
            return 0;
        ...
        Debug.Log($"[EXP-SWARM-003 Metrics] episode_id={episodeId} -> {_filePath}");
        return episodeId;
    }
    ...
    static string FormatRow(int episodeId, in ExperimentSwarm003EpisodeMetrics metrics)
    {
        return string.Join(",",
            episodeId.ToString(CultureInfo.InvariantCulture),
            metrics.seed.ToString(CultureInfo.InvariantCulture),
            EscapeCsvField(metrics.explorationMode ?? string.Empty),
            ...
            metrics.newVoxelsDiscovered.ToString(CultureInfo.InvariantCulture),
            metrics.revisitRatio.ToString("F3", CultureInfo.InvariantCulture),
            metrics.meanVisitCount.ToString("F3", CultureInfo.InvariantCulture),
            metrics.explorationEfficiency.ToString("F3", CultureInfo.InvariantCulture),
            metrics.frontierBiasSteps.ToString(CultureInfo.InvariantCulture),
