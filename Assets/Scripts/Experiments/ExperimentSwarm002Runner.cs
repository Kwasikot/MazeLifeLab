...
public class ExperimentSwarm002Runner : MonoBehaviour
{
    ...
    [SerializeField] string metricsCsvRelativePath = "results/experiment_swarm_002_obstacles.csv";
    ...
    [Header("Obstacles")]
    [SerializeField] int obstacleCount = 16;
    [SerializeField] bool anchorObstaclesToMazeWalls = true;
    [SerializeField] float obstacleRadius = 2.4f;
    [SerializeField] float obstacleHeight = 14f;
    [SerializeField] float obstacleWallThickness = 0.55f;
    [SerializeField] int obstacleWallSpanCells = 4;
    [SerializeField] float obstacleWallLengthScale = 1f;
    [SerializeField] float obstacleAvoidanceDistance = 5f;
    [SerializeField] float obstacleAvoidanceWeight = 18f;
    [SerializeField] float obstacleContactDistance = 0.35f;
    [SerializeField] Color obstacleColor = new Color(0.2f, 0.2f, 0.2f);
    ...
    readonly List<SwarmFlightObstacle> _obstacles = new List<SwarmFlightObstacle>();
    readonly List<MazeWall> _visibleWallBuffer = new List<MazeWall>();
    readonly HashSet<int> _visitedVoxels = new HashSet<int>();
    readonly HashSet<int> _usedObstacleCells = new HashSet<int>();
    readonly HashSet<int> _usedObstacleWalls = new HashSet<int>();

    ExperimentSwarm002MetricsLogger _metricsLogger;
    Transform _agentsRoot;
    Transform _obstaclesRoot;
    System.Random _rng;
    Vector3 _activeArenaCenter;
    Vector3 _activeArenaSize;
    int _steps;
    bool _isRunning;
    int _nearObstacleSteps;
    int _obstacleContacts;
    float _obstacleDistanceSum;
    int _obstacleDistanceSamples;
    float _fragmentationSum;
    ...
    public int ObstacleContacts => _obstacleContacts;
    public int NearObstacleSteps => _nearObstacleSteps;
    public float MeanObstacleDistance { get; private set; }
    public float FragmentationIndex { get; private set; }
    ...
    void Awake()
    {
        ...
        _metricsLogger = new ExperimentSwarm002MetricsLogger(metricsCsvRelativePath, enableMetricsLogging);
    ...
    void OnValidate()
    {
        ...
        obstacleCount = Mathf.Clamp(obstacleCount, 1, 256);
        obstacleRadius = Mathf.Max(0.1f, obstacleRadius);
        obstacleHeight = Mathf.Max(0.5f, obstacleHeight);
        obstacleWallThickness = Mathf.Max(0.1f, obstacleWallThickness);
        obstacleWallSpanCells = Mathf.Clamp(obstacleWallSpanCells, 1, 12);
        obstacleWallLengthScale = Mathf.Clamp(obstacleWallLengthScale, 0.1f, 2f);
        obstacleAvoidanceDistance = Mathf.Max(0f, obstacleAvoidanceDistance);
        obstacleAvoidanceWeight = Mathf.Max(0f, obstacleAvoidanceWeight);
        obstacleContactDistance = Mathf.Max(0f, obstacleContactDistance);
    ...
    public void BeginEpisode()
    {
        ...
        {
            Debug.LogWarning("[EXP-SWARM-002] Runner is disabled; enabling before starting.");
            enabled = true;
        }
        ...
        _isRunning = true;
        Debug.Log(
            $"[EXP-SWARM-002] Started seed={seed} agents={_agents.Count} obstacles={_obstacles.Count} " +
    ...
    void EndEpisode(EpisodeTerminationReason reason)
    {
        ...
        _metricsLogger.LogEpisode(new ExperimentSwarm002EpisodeMetrics
        {
            seed = seed,
            agentCount = _agents.Count,
            obstacleCount = _obstacles.Count,
            steps = _steps,
            meanSpeed = MeanSpeed,
            ...
            obstacleContacts = ObstacleContacts,
            nearObstacleSteps = NearObstacleSteps,
            meanObstacleDistance = MeanObstacleDistance,
            fragmentationIndex = FragmentationIndex,
            terminationReason = reason
        });

        Debug.Log(
            $"[EXP-SWARM-002] Ended reason={reason} steps={_steps} contacts={ObstacleContacts} " +
            $"meanObstacleDistance={MeanObstacleDistance:F2} fragmentation={FragmentationIndex:F2}.");
    }

    void ResetMetrics()
    {
        _steps = 0;
        _nearObstacleSteps = 0;
        _obstacleContacts = 0;
        _obstacleDistanceSum = 0f;
        _obstacleDistanceSamples = 0;
        _fragmentationSum = 0f;
        MeanSpeed = 0f;
        ...
        MeanObstacleDistance = 0f;
        FragmentationIndex = 0f;
    ...
    void EndEpisodeWithoutLogging()
    {
        ...
        _obstacles.Clear();
        _visitedVoxels.Clear();
        _usedObstacleCells.Clear();
        _usedObstacleWalls.Clear();
    }
    ...
    void EnsureAgentsRoot()
    {
        ...
        var root = new GameObject("EXP-SWARM-002 Agents");
        root.transform.SetParent(transform, false);
        _agentsRoot = root.transform;
    }

    void EnsureObstaclesRoot()
    {
        if (_obstaclesRoot != null && _obstaclesRoot.gameObject != null)
            return;

        var root = new GameObject("EXP-SWARM-002 Obstacles");
        root.transform.SetParent(transform, false);
        _obstaclesRoot = root.transform;
    }
    ...
    void SpawnAgents()
    {
        ...
        for (int i = 0; i < agentCount; i++)
        {
            ...
            agentObject.name = $"Swarm002Agent_{i:00}";
        ...
        Debug.Log($"[EXP-SWARM-002] Spawned {_agents.Count} boids under '{_agentsRoot.name}'.");
    }
    ...
    void SpawnObstacles()
    {
        ...
        _usedObstacleWalls.Clear();
        ClearObstacleObjects();
        CollectVisibleMazeWalls();

        for (int i = 0; i < obstacleCount; i++)
        {
            SwarmFlightObstacle obstacle = RandomObstacle();
            _obstacles.Add(obstacle);

            GameObject obstacleObject = GameObject.CreatePrimitive(PrimitiveType.Cube);
            obstacleObject.name = $"Swarm002Obstacle_{i:00}";
            obstacleObject.transform.SetParent(_obstaclesRoot, true);
            obstacleObject.transform.position = obstacle.Center;
            obstacleObject.transform.localScale = new Vector3(
                obstacle.HalfExtentsXZ.x * 2f,
                obstacle.Height,
                obstacle.HalfExtentsXZ.y * 2f);

            var collider = obstacleObject.GetComponent<Collider>();
            if (collider != null)
                Destroy(collider);

            ExperimentAgentVisuals.ApplyUnlitColor(obstacleObject.GetComponent<Renderer>(), obstacleColor);
        }

        Debug.Log($"[EXP-SWARM-002] Spawned {_obstacles.Count} obstacle markers.");
    }

    void CollectVisibleMazeWalls()
    {
        _visibleWallBuffer.Clear();
        if (!anchorObstaclesToMazeWalls || !useMazeBounds || !TryGetMazeGenerator(out MazeGenerator generator))
            return;

        foreach (KeyValuePair<string, MazeWall> entry in generator.WallsMap)
        {
            if (entry.Value.bVisible)
                _visibleWallBuffer.Add(entry.Value);
        }
    }

    void ClearObstacleObjects()
    {
        ...
        {
            Transform child = _obstaclesRoot.GetChild(i);
            if (child != null)
    ...
    SwarmFlightObstacle RandomObstacle()
    {
        if (anchorObstaclesToMazeWalls && TryBuildWallAnchoredObstacle(out SwarmFlightObstacle wallObstacle))
            return wallObstacle;

        Vector3 center = RandomObstaclePosition();
        return new SwarmFlightObstacle(center, obstacleRadius, obstacleHeight);
    }

    bool TryBuildWallAnchoredObstacle(out SwarmFlightObstacle obstacle)
    {
        obstacle = new SwarmFlightObstacle();
        if (_visibleWallBuffer.Count == 0)
            return false;

        int maxAttempts = Mathf.Max(64, obstacleCount * 8);
        for (int attempt = 0; attempt < maxAttempts; attempt++)
        {
            int wallIndex = _rng.Next(_visibleWallBuffer.Count);
            if (!_usedObstacleWalls.Add(wallIndex))
                continue;

            MazeWall wall = _visibleWallBuffer[wallIndex];
            Vector3 center = (wall.A + wall.B) * 0.5f;
            center.y = _activeArenaCenter.y;

            Vector3 wallDelta = wall.B - wall.A;
            wallDelta.y = 0f;
            float wallLength = Mathf.Max(
                0.1f,
                wallDelta.magnitude * obstacleWallLengthScale * obstacleWallSpanCells);
            bool alongX = Mathf.Abs(wallDelta.x) >= Mathf.Abs(wallDelta.z);
            Vector2 halfExtents = alongX
                ? new Vector2(wallLength * 0.5f, obstacleWallThickness)
                : new Vector2(obstacleWallThickness, wallLength * 0.5f);

            obstacle = new SwarmFlightObstacle(center, halfExtents, obstacleHeight);
            return true;
        }
    ...
    Vector3 RandomObstaclePosition()
    {
        if (useMazeBounds && TryGetMazeGenerator(out MazeGenerator generator))
        {
            int maxAttempts = Mathf.Max(64, obstacleCount * 8);
            for (int attempt = 0; attempt < maxAttempts; attempt++)
            {
                int cellX = _rng.Next(generator.Config.mazeWidthCells);
                int cellY = _rng.Next(generator.Config.mazeHeightCells);
                int key = cellX * 10000 + cellY;
                if (!_usedObstacleCells.Add(key))
                    continue;

                Vector3 position = generator.GetCellCenterWorld(cellX, cellY);
                position.y = _activeArenaCenter.y;
                return position;
            }
        }
    ...
    void TrackSwarmShapeMetrics(Vector3 center)
    {
        ...
        float currentFragmentation = _agents.Count > 0
            ? (float)isolatedAgents / _agents.Count
            : 0f;
        _fragmentationSum += currentFragmentation;
        FragmentationIndex = _steps > 0 ? _fragmentationSum / _steps : currentFragmentation;
    }

    void TrackObstacleMetrics()
    {
        ...
        for (int i = 0; i < _agents.Count; i++)
        {
            float nearest = NearestObstacleSurfaceDistance(_agents[i].transform.position);
            _obstacleDistanceSum += nearest;
            _obstacleDistanceSamples++;

            if (nearest <= obstacleAvoidanceDistance)
                _nearObstacleSteps++;

            if (nearest <= obstacleContactDistance)
                _obstacleContacts++;
        }

        MeanObstacleDistance = _obstacleDistanceSamples > 0
            ? _obstacleDistanceSum / _obstacleDistanceSamples
            : 0f;
    }

    float NearestObstacleSurfaceDistance(Vector3 position)
    {
        float nearest = float.PositiveInfinity;
        for (int i = 0; i < _obstacles.Count; i++)
        {
            SwarmFlightObstacle obstacle = _obstacles[i];
            Vector3 offset = position - obstacle.Center;
            float outsideX = Mathf.Max(Mathf.Abs(offset.x) - obstacle.HalfExtentsXZ.x, 0f);
            float outsideZ = Mathf.Max(Mathf.Abs(offset.z) - obstacle.HalfExtentsXZ.y, 0f);
            float surfaceDistance = Mathf.Sqrt(outsideX * outsideX + outsideZ * outsideZ);
            if (surfaceDistance < nearest)
                nearest = surfaceDistance;
        }

        return float.IsInfinity(nearest) ? 0f : nearest;
    }
