...
public struct Experiment006EpisodeMetrics
{
    public int mazeSeed;
    ...
    public int swarmEdgesDeposited;
    public int swarmGraftNodes;
    public EpisodeTerminationReason terminationReason;
}

public sealed class Experiment006MetricsLogger
{
    const string CsvHeader =
        "episode_id,maze_seed,algorithm,swarm_mode,agent_count,success,steps,steps_to_first_goal,agents_at_goal," +
        "total_collisions,total_path_length,team_coverage_percent,overlap_percent,swarm_edges_deposited,swarm_graft_nodes,termination_reason";

    readonly string _filePath;
    readonly object _writeLock = new object();
    int _nextEpisodeId;

    public Experiment006MetricsLogger(string relativePath, bool enabled = true)
    {
        Enabled = enabled;
    ...
    public int LogEpisode(in Experiment006EpisodeMetrics metrics)
    {
        if (!Enabled)
            return 0;
        ...
        Debug.Log($"[EXP-006 Metrics] episode_id={episodeId} -> {_filePath}");
        return episodeId;
    }
    ...
    static string FormatRow(int episodeId, in Experiment006EpisodeMetrics metrics)
    {
        return string.Join(",",
            ...
            EscapeCsvField(metrics.swarmMode ?? string.Empty),
            ...
            metrics.swarmEdgesDeposited.ToString(CultureInfo.InvariantCulture),
            metrics.swarmGraftNodes.ToString(CultureInfo.InvariantCulture),
